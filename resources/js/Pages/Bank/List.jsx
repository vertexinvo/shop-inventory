        ...
        <div className="flex items-center justify-between py-2">
          ...
          <div className="flex items-center space-x-3">
            <MdKeyboardBackspace
              ...
              title="Back"
            />
            <h2 className="font-semibold text-xl text-gray-800 leading-tight">Bank</h2>
          </div>
        </div>
      }
    >
        ...
        <div className="w-full ">
          <div className="flex flex-col md:flex-row justify-end items-center mt-2 mb-4">
            ...
            <div className="flex flex-col md:flex-row w-full md:justify-start space-y-2 md:space-y-0 md:space-x-2">
              ...
              <button
                onClick={() => router.get(route('bank.create'))}
              ...
              {/* <Formik
                ...
                onSubmit={(values) => {
                  router.get(route('bank.index'), { search: values.search }, { preserveState: true });
                }}
              >
          ...
          <div className="overflow-x-auto">
                  ...
                  <tr className='text-xs font-semibold tracking-wide text-left text-white uppercase border-b bg-black'>
                    <th class="pl-4 w-8">
                      <input id="checkbox" type="checkbox" class="hidden peer"
                        onChange={(e) => setSelectId(e.target.checked ? banks.data.map((bank) => bank.id) : [])}
                        checked={selectId.length === banks.data.length}
                      />
                      <label for="checkbox"
                    ...
                    <th class="p-4 text-right text-sm font-semibold ">
                      Account Type
                    </th>
                    <th class="p-4 text-right text-sm font-semibold ">
                    Swift Code
                ...
                <tbody class="whitespace-nowrap">
                  ...
                  {banks.data.map((bank, index) => (
                    ...
                    <tr class="odd:bg-gray-50">
                      <td key={bank.id} className="pl-4 w-8">
                        <input
                          id={`checkbox-${bank.id}`}
                          type="checkbox"
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectId((prev) => [...prev, bank.id]); // Add user ID to state
                            } else {
                              setSelectId((prev) => prev.filter((id) => id !== bank.id)); // Remove user ID from state
                            }
                          }}
                          checked={selectId.includes(bank.id)}

                        // Bind state to checkbox
                        />
                        <label
                          htmlFor={`checkbox-${bank.id}`} // Match label with checkbox id
                     ...
                     <td className="p-4 text-sm">{bank.name || 'N/A'}</td>
                    <td className="p-4 text-sm">{bank.current_balance || '0.00'}</td>
                    <td className="p-4 text-sm">{bank.account_number || 'N/A'}</td>
                    <td className="p-4 text-sm">{bank.iban || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.account_type || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.swift_code || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.routing_number || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.bank_address || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.bank_phone || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.bank_email || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.bank_website || 'N/A'}</td>
                    <td className="p-4 text-sm text-right">{bank.notes || '—'}</td>
                    <td className="p-4 text-sm text-right">{bank.status || 'Inactive'}</td>
                    <td className="p-4 text-sm text-right">{bank.created_at ? new Date(bank.created_at).toLocaleDateString() : 'N/A'}</td>


                    <td className="flex justify-end py-2 px-4 border-b border-gray-200 text-right">
                        <button onClick={() => router.get(route('bank.edit', bank.id))} className="flex mr-4 items-center space-x-2 bg-blue-500 hover:bg-blue-600 rounded text-white px-3 py-2" title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-black text-white"
                        ...
                        <button onClick={() => setIsDeleteModalOpen(bank)} title="Delete" className="flex mr-4 items-center space-x-2 bg-red-500 hover:bg-red-600 rounded text-white px-3 py-2">
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-black text-white" viewBox="0 0 24 24">
                            <path
            ...
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9    ">
              <span class="flex items-center col-span-3"> Showing {banks.from} - {banks.to} of {banks.total} </span>
              ...
              <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                <nav aria-label="Table navigation">
                  <ul class="inline-flex items-center">
                    ...
                    <li>
                      <button onClick={() => banks.links[0].url ? router.get(banks.links[0].url) : null} class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-purple" aria-label="Previous">
                    ...
                    {(() => {
                      ...
                      const activeIndex = banks.links.findIndex((l) => l.active);
                    ...
                    <li>
                      <button onClick={() => banks.links[banks.links.length - 1].url && window.location.assign(banks.links[banks.links.length - 1].url)} class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple" aria-label="Next">
      ...
      <ConfirmModal isOpen={isDeleteModalOpen !== null} onClose={() => setIsDeleteModalOpen(null)} title="Are you sure you want to delete?" onConfirm={() => {
        ...
        router.delete(route('bank.destroy', isDeleteModalOpen.id), {
          preserveScroll: true,
          preserveState: true,
        })
      ...
      <ConfirmModal isOpen={isBulkDeleteModalOpen} onClose={() => setIsBulkDeleteModalOpen(false)} title="Are you sure you want to delete these users?" onConfirm={() => {
        ...
        router.post(route('bank.bulkdestroy'), { ids: selectId.join(',') }, {
          onSuccess: () => {
