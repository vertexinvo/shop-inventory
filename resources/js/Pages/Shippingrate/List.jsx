...
export default function List(props) {
  ...
  const { auth, shippingrates } = props
  ...
  return (
    ...
    >
        ...
        <div className="w-full ">
          ...
          <div className="overflow-x-auto">
            <div class="font-[sans-serif] overflow-x-auto">
              <table class="min-w-full bg-white">
                  ...
                  <tr className='text-xs font-semibold tracking-wide text-left text-white uppercase border-b bg-black'>
                    <th class="pl-4 w-8">
                      <input id="checkbox" type="checkbox" class="hidden peer"
                        onChange={(e) => setSelectId(e.target.checked ? shippingrates.data.map((tax) => tax.id) : [])}
                        checked={selectId.length === shippingrates.data.length}
                      />
                      <label for="checkbox"
                ...
                <tbody class="whitespace-nowrap">
                  ...
                  {shippingrates.data.map((shippingrate, index) => (
                    <tr key={shippingrate.id} class="odd:bg-gray-50">
                      <td className="pl-4 w-8">
                        <input
                          id={`checkbox-${shippingrate.id}`} // Unique id for each checkbox
                          type="checkbox"
                          className="hidden peer"
                          value={shippingrate.id}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectId((prev) => [...prev, shippingrate.id]); // Add user ID to state
                            } else {
                              setSelectId((prev) => prev.filter((id) => id !== shippingrate.id)); // Remove user ID from state
                            }
                          }}
                          checked={selectId.includes(shippingrate.id)} // Bind state to checkbox
                        />
                        <label
                          htmlFor={`checkbox-${shippingrate.id}`} // Match label with checkbox id
                      ...
                      <td class="p-4 text-sm">
                        <div class="flex items-center cursor-pointer w-max">
                          ...
                          <div class="ml-4">
                            <p class="text-sm text-black">{shippingrate.area_name}</p>
                          </div>
                        </div>
                      </td>
                      <td class="p-4 text-sm text-black">
                        {shippingrate.postal_code || 'N/A'}
                      </td>
                      <td class="p-4 text-sm text-black">
                        {shippingrate.city_name || 'N/A'}
                      </td>
                      <td class="p-4 text-sm text-black">
                        {shippingrate.state_name || 'N/A'}
                      </td>
                      <td class="p-4 text-sm text-black">
                        {shippingrate.country_name || 'N/A'}
                      </td>
                      <td class="p-4 text-sm text-black">
                        {shippingrate.fee || 'N/A'}
                      </td>

                      <td className="flex justify-end py-2 px-4 border-b border-gray-200 text-right">
                        <button onClick={() => router.get(route('shippingrate.edit', shippingrate.id))} className="flex mr-4 items-center space-x-2 bg-blue-500 hover:bg-blue-600 rounded text-white px-3 py-2" title="Edit">
                        ...
                        <button onClick={() => setIsDeleteModalOpen(shippingrate)} title="Delete" className="flex mr-4 items-center space-x-2 bg-red-500 hover:bg-red-600 rounded text-white px-3 py-2">
            ...
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9    ">
              <span class="flex items-center col-span-3"> Showing {shippingrates.from} - {shippingrates.to} of {shippingrates.total} </span>
              ...
              <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                <nav aria-label="Table navigation">
                  <ul class="inline-flex items-center">
                    ...
                    <li>
                      <button onClick={() => shippingrates.links[0].url ? router.get(shippingrates.links[0].url) : null} class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-purple" aria-label="Previous">
                    ...
                    {(() => {
                      ...
                      const activeIndex = shippingrates.links.findIndex((l) => l.active);

                      return shippingrates.links
                    ...
                    <li>
                      <button onClick={() => shippingrates.links[shippingrates.links.length - 1].url && window.location.assign(shippingrates.links[shippingrates.links.length - 1].url)} class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple" aria-label="Next">
      ...
      <ConfirmModal isOpen={isDeleteModalOpen !== null} onClose={() => setIsDeleteModalOpen(null)} title="Are you sure you want to delete?" onConfirm={() => {
        ...
        router.delete(route('shippingrate.destroy', isDeleteModalOpen.id), {
          preserveScroll: true,
          preserveState: true,
        })
      ...
      <ConfirmModal isOpen={isBulkDeleteModalOpen} onClose={() => setIsBulkDeleteModalOpen(false)} title="Are you sure you want to delete these Shipping rates?" onConfirm={() => {

        router.post(route('shippingrate.bulkdestroy'), { ids: selectId.join(',') }, {
          onSuccess: () => {
