            ...
            header={
                <>
                    <MdKeyboardBackspace
                        ...
                        title="Back"
                    /><h2 className="font-semibold text-xl text-gray-800 leading-tight">Add Expense</h2>
                </>}
        >
            <Head title="Expense" />
                                ...
                                <div className="w-full lg:w-full mx-auto bg-white rounded shadow">
                                    <Formik enableReinitialize initialValues={{ name: brand?.name || '', description: brand?.description || '' }}
                                        ...
                                        onSubmit={(values, { setSubmitting, resetForm }) => {
                                            brand ?
                                                router.put(route('expence.update', brand.id), values, { onSuccess: () => resetForm(), preserveState: true, preserveScroll: true })
                                                :
                                                router.post(route('expence.store'), values, { onSuccess: () => resetForm(), preserveState: true, preserveScroll: true })
                                            ...
                                            <Form>
                                                <div className="py-4 px-8">
                                                    ...
                                                    <div className="flex items-center justify-start gap-1 mt-8">
                                                        ...
                                                        <button onClick={() =>  router.get(route('expense.index'))} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg" type="button">
                                                            Close
                                                        </button>
                                                    </div>
                                                </div>
