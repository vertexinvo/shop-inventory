      ...
      header={
        <>
          <MdKeyboardBackspace
            ...
            onClick={() => router.get(route('tax.index'))}
            title="Back"
          /><h2 className="font-semibold text-xl text-gray-800 leading-tight">Add Tax</h2>
        </>}
    >
      <Head title="Tax" />
            ...
            <div class="w-full bg-grey-lightest">
              <div class="container mx-auto py-3 px-5">
                <div class="w-full lg:w-full mx-auto bg-white rounded shadow">
                  <Formik enableReinitialize initialValues={{ name: '', cost: '' }}
                    validationSchema={Yup.object({
                      name: Yup.string().required('Name is required'),
                      cost: Yup.string().required('Cost is required'),
                    })}
                    onSubmit={(values, { setSubmitting, resetForm }) => {
                      router.post(route('tax.store'), values, { onSuccess: () => resetForm() });
                    }}
                  >
                    <Form>
