  ...
  return (
    ...
    >
        ...
        <div className="w-full ">
          ...
          <div className="overflow-x-auto">
              ...
              <table class="min-w-full bg-white">
                  ...
                  <tr className='text-xs font-semibold tracking-wide text-left text-white uppercase border-b bg-black'>
                    <th class="pl-4 w-8">
                      <input id="checkbox" type="checkbox" class="hidden peer"
                        onChange={(e) => setSelectId(e.target.checked ? categories.data.map((category) => category.id) : [])}
                        checked={selectId.length === categories.data.length}
                      />
                      <label for="checkbox"
                ...
                <tbody class="whitespace-nowrap">
                  ...
                  {categories.data.map((category, index) => (
                    ...
                    <tr key={category.id} class="odd:bg-gray-50">
                      <td className="pl-4 w-8">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectId((prev) => [...prev, category.id]); // Add user ID to state
                            } else {
                              setSelectId((prev) => prev.filter((id) => id !== category.id)); // Remove user ID from state
                            }
                          }}
                          checked={selectId.includes(category.id)}

                        // Bind state to checkbox
                        />
                        <label
                          htmlFor={`checkbox-${category.id}`} // Match label with checkbox id
                      ...
                      <td className="flex justify-end py-2 px-4 border-b border-gray-200 text-right">
                        <button onClick={() => router.get(route('category.edit', category.id))} className="flex mr-4 items-center space-x-2 bg-blue-500 hover:bg-blue-600 rounded text-white px-3 py-2" title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current text-white"
                        ...
                        <button onClick={() => setIsDeleteModalOpen(category)} title="Delete" className="flex mr-4 items-center space-x-2 bg-red-500 hover:bg-red-600 rounded text-white px-3 py-2">
            ...
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9    ">
              <span class="flex items-center col-span-3"> Showing {categories.from} - {categories.to} of {categories.total} </span>
              ...
              <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                <nav aria-label="Table navigation">
                  <ul class="inline-flex items-center">
                    ...
                    <li>
                      <button onClick={() => categories.links[0].url ? router.get(categories.links[0].url) : null} class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-purple" aria-label="Previous">
                    ...
                    {(() => {
                      ...
                      const activeIndex = categories.links.findIndex((l) => l.active);
                    ...
                    <li>
                      <button onClick={() => categories.links[categories.links.length - 1].url && window.location.assign(categories.links[categories.links.length - 1].url)} class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple" aria-label="Next">
      ...
      <ConfirmModal isOpen={isDeleteModalOpen !== null} onClose={() => setIsDeleteModalOpen(null)} title="Are you sure you want to delete?" onConfirm={() => {
        ...
        router.delete(route('category.destroy', isDeleteModalOpen.id), {
          preserveScroll: true,
          preserveState: true,
        })
